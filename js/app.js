...
class App
{
    ...
    iniciar()
    {
        ...
        this.animador = window.setInterval(this.vista.moverBolas.bind(this.vista), 100); // nuestra animador llamara cada cierto intervalo de tiempo a la funcion de movimiento, el animador pertenece al controlador y verlas moviendose a la vista
    }
    verificar(evento)
    {
...
class Vista
{
    ...
    moverBolas()
    {
        for (let indice = 0; indice < this.bolas.length; indice++)
        {
            ...
            if (top <= 0+10 || top >= this.contenedor.clientHeight -bola.div.clientHeight-10 ) //le restamos el tamaño de la bola para que coincida con el tamaño del contenedor
            {
                this.bolas[indice].velocidady = -this.bolas[indice].velocidady //modificamos el signo de la velocidad si tocan los bordes
            }
            if (left <= 0+10 || left >= this.contenedor.clientWidth -bola.div.clientWidth-10 ) //le restamos el tamaño de la bola para que coincida con el tamaño del contenedor
            {
...
class Bola
{
    constructor(contenedor,controlador)
    {
        ...
        this.div.appendChild(document.createTextNode((Math.floor(Math.random() * (this.maxBola - this.minBola)) + this.minBola)));
...
// App.limiteNumero = 100; //atributo estatico
